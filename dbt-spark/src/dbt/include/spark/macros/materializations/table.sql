...
# Function to load DataFrame from table name or SQL query, this is used in source() and ref() functions when a microbatch model reference a model with an config of loaded_at_field 
...
import os
# If this os environment variable is not set, pyarrow will by default convert timestamps to UTC
# which can cause issues when converting pandas DataFrames with localized timestamps to Spark DataFrames
if "PYARROW_IGNORE_TIMEZONE" not in os.environ:
  os.environ["PYARROW_IGNORE_TIMEZONE"] = "1"

# Override source anf ref function to support dbt_load_df_function and dbt_sql_df_function
